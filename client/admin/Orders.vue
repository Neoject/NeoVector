...
export default {
  ...
  mounted() {
    setPageTitle(this.params.title, 'заказы');
  ...
  methods: {
    ...
    async loadOrders() {
      ...
      try {
        ...
        if (response.ok) this.orders = await response.json();
        else this.ordersError = 'Ошибка загрузки заказов';
      } catch {
    ...
    async updatePaymentStatus(orderId, paymentStatus) {
      try {
        const response = await api.updatePaymentStatus(orderId, paymentStatus);
        const result = await response.json();
    ...
    async deleteOrder(orderId) {
      if (!orderId || !confirm('Вы действительно хотите удалить этот заказ?')) return false;
