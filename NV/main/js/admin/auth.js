const Auth = {
    ...
    computed: {
        isAuthenticated() {
            const adminAuth = typeof window !== 'undefined' && window.__ADMIN_AUTH__;
            return !!(adminAuth && adminAuth.authenticated === true && adminAuth.role === 'admin');
        }
    },
    methods: {
        openLogin() {
            // Метод для открытия формы входа (может быть переопределен в компоненте)
        },
        closeLogin() {
            // Метод для закрытия формы входа (может быть переопределен в компоненте)
        },
        doLogin() {
            // Метод для выполнения входа (может быть переопределен в компоненте)
        },
        async register() {
            this.loginError = '';
            this.registerError = '';
