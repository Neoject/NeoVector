...
export default {
  ...
  methods: {
    ...
    // ── Colors ──────────────────────────────────────────────────
    loadColors() {
      fetch('../api.php?action=get_colors', { credentials: 'same-origin' })
    ...
    async saveColors() {
      ...
      try {
        ...
        if (response.ok && data.success) {
          ...
          alert('Ошибка при сохранении цветов: ' + (data.error || 'Неизвестная ошибка'));
        }
      } catch (error) {
        alert('Ошибка: ' + error);
      } finally {
    ...
    resetColors() {
      ...
      fetch('../api.php?action=reset_colors', { credentials: 'same-origin' })
          .then(r => r.ok ? r.json() : null)
          .then(() => {
            COLOR_GROUPS.forEach(group => {
    ...
    clearFile() {
      if (confirm('Вы действительно хотите удалить лого?')) {
        ...
        fetch('../api.php?action=delete_logo').then(() => null);
      }
    },
    formatSize(b) {
          ...
          <div class="form-group">
            ...
            >
              ...
              <div class="file-upload-area">
                ...
                <template v-else>
                  <span class="file-drop-zone__link">выберите файл</span>
                  <div class="file-drop-zone__hint">PNG, JPG, SVG, WEBP · максимум 5 МБ</div>
                </template>
              </div>
            </div>
            <button
        ...
        <div class="color-grid">
          ...
          >
            ...
            <div
                ...
                :style="{ background: colors[variable.key] }"
            />
...
.color-settings { margin-top: 2rem; }
...
/* ── Color grid ────────────────────────────────────────── */
.color-grid { display: flex; flex-direction: column; gap: .45rem; }
...
.btn-outline:hover { background: var(--primary, #6d7bba); color: #fff; }
...
.btn-danger-outline:hover { background: var(--error-bg, #dc3545); color: #fff; }
</style>