...
export default {
  ...
  methods: {
    ...
    async hideProduct(id) {
      ...
      const fd = new FormData()
      fd.append('action', 'visibility')
      fd.append('id', id)
      fd.append('visibility', nv)
    ...
    async saveCategory() {
      ...
      try {
        ...
        fd.append('slug', this.categoryForm.slug || '')
        fd.append('sort_order', this.categoryForm.sort_order || 0)
        const r = await fetch('../api.php', { method: 'POST', body: fd, credentials: 'same-origin' })
    ...
    async saveCategoriesOrder() {
      ...
      fd.append('categories_order', JSON.stringify(this.categories.map(c => c._id)))
      await fetch('../api.php', { method: 'POST', body: fd, credentials: 'same-origin' })
    },
    ...
    async saveProduct() {
      try {
        ...
        if (this.editingProduct) {
          ...
          fd.append('id', this.editingProduct.id)
        } else {
    ...
    async handleAdditionalImagesSelect(e) {
      ...
      const fd = new FormData()
      fd.append('action', 'add_product_images')
    ...
    async generateDescriptionWithAI() {
      ...
      try {
        const fd = new FormData()
        fd.append('action', 'generate_product_description')
        fd.append('name', this.productForm.name.trim())
        const r = await fetch('../api.php', { method: 'POST', body: fd, credentials: 'same-origin' })
        const d = await r.json()
